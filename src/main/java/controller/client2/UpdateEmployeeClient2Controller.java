...
public class UpdateEmployeeClient2Controller extends HttpServlet {
    ...
    public UpdateEmployeeClient2Controller() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		List<Person> listUser = personDAO.getListUserClient2();
		...
		for(int i = 0; i<listEmployee.size();i++) {
			...
			if(ktr) {
				listEmployee.get(i).setUser_Forget(UpdateEmployeeClient2Controller.randomAlphaNumeric(8));
				listEmployee.get(i).setUser_Password("123456");
				listEmployee.get(i).setUser_Active(1);
				if(listEmployee.get(i).getUser_Role() == 1) {
					personDAO.addUserClient1(listEmployee.get(i));
				}else {
		...
		response.sendRedirect(request.getContextPath() + "/client2/admin/user/index?msg=1");
		return;
