...
public class ReadExcelNhanVien {
	...
	public static void main(String[] args) throws IOException {
		...
		final List<Person> persons = readExcel(excelFilePath);
		for (Person it : persons) {
			System.out.println(it.getUser_Email());
		}
	}
		...
		while (iterator.hasNext()) {
			...
			while (cellIterator.hasNext()) {
				...
				case COLUMN_INDEX_Name:
					person.setUser_Name((String) getCellValue(cell));
					;
					;
					break;
				case COLUMN_INDEX_Email:
					person.setUser_Email(((String) getCellValue(cell)));
					;
					break;
				case COLUMN_INDEX_Role:
					person.setUser_Role(new BigDecimal((double) cellValue).intValue());;
