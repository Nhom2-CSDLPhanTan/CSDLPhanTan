                ...
                <div class="col-sm-12 col-xl-6">
                    <div class="bg-secondary rounded h-100 p-4" style="margin-left: 150px; width: 850px;">
                    	...
                    	switch(msg){
                    		case 1:
                    			%>
                    			<p style="color: green;font-style:italic">Thêm hình ảnh thành công</p>
                    			<%
                    			break;
                    			
                    		case 2:
                    			%>
                    			<p style="color: green;font-style:italic">Xóa hình ảnh thành công</p>
                    			<%
                    			break;
                    		case 3:
                    			%>
                    			<p style="color: green;font-style:italic">Sửa hình ảnh thành công</p>
                    			<%
                    			break;
                    		}
                    	%>
                        ...
                        <table class="table table-striped">
                            ...
                            <tbody>
                            	...
                            	String idsp = "";
                            	if(request.getAttribute("idsp") != null){
                            		idsp = (String)request.getAttribute("idsp");
                            	}
                            	if(request.getAttribute("listPicture") != null){
                            		listPicture = (ArrayList<Picture>)request.getAttribute("listPicture");
                            	}
                            	...
                            	<tr>
                            		<td><%=i %></td>
                            		<td>
                            			<img width="150px" height="100px" border="1" class="img_1" src="<%=request.getContextPath()%>/file/<%=item.getUrl()%>" alt="G-Shock ">
                            		</td>
                            		<td>
                                                <a href="<%=request.getContextPath()%>/client1/admin/sanpham/hinhanhsp/edit?idha=<%=item.getId()%>&idsp=<%=item.getId_Pro()%>"><button type="button" class="btn btn-info m-2">Sửa</button></a>
                                                <a href="<%=request.getContextPath()%>/client1/admin/sanpham/hinhanhsp/del?idha=<%=item.getId()%>&idsp=<%=item.getId_Pro()%>"><button type="button" class="btn btn-primary m-2" onclick="return confirm('Bạn có chắc muốn xóa không');">Xóa</button></a>
                                            </td>
                            	</tr>
                            	<%
                            	}}
                            	%>
                            	<tr >
                            		<td colspan="3"><a href="<%=request.getContextPath() %>/client1/admin/sanpham/hinhanhsp/add?idsp=<%=idsp %>" style="color: blue">+ Thêm hình ảnh</a></td>
                            	</tr>
